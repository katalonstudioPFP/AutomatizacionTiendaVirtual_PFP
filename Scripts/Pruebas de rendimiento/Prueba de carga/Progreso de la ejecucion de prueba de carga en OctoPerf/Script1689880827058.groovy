...
for(int i=0;i<=1000 ; i++) {
 ...
 response=WS.sendRequest(findTestObject('Object Repository/Solicitudes a las pruebas de carga/Postman/SeguimientoDeLaPruebaDeCarga', [('benchReportId') : GlobalVariable.BENCH_REPORT_ID]))
...
String ProgresoDeLaEjecucion=WS.getElementPropertyValue(response,'value').toString()
 ...
 if(ProgresoDeLaEjecucion=="100.0") {
	...
	println("LA EJECUCION DE LA PRUEBA DE CARGA HA TERMINADO")
	println("AHORA PROCEDEMOS A GENERAR Y DESCARGAR EL INFORME DE PRUEBA DE CARGA EN PDF")
	//se termina el bucle
	break
  }
  else {
